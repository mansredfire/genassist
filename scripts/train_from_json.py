...
import argparse
from src.collectors.json_importer import JSONImporter
from src.training.pipeline import TrainingPipeline


def main():
    parser = argparse.ArgumentParser(description='Train BugPredict AI from JSON file')
    ...
    parser.add_argument('--validate-only', action='store_true', help='Only validate JSON, don\'t train')
    parser.add_argument('--quick', action='store_true', help='Quick training mode (faster, less accurate)')
    
    args = parser.parse_args()
    
    print("=" * 70)
    print("BugPredict AI - JSON Training")
    print("=" * 70)
    ...
    # Validate JSON
    print(f"\nValidating JSON file: {args.input}")
    validation = importer.validate_json(args.input)
    
    if not validation['valid']:
        print(f"\n❌ JSON validation failed: {validation['error']}")
        if 'found_fields' in validation:
            print(f"Found fields: {validation['found_fields']}")
        sys.exit(1)
    
    print(f"✓ JSON is valid")
    print(f"  Reports: {validation['reports']}")
    ...
    reports = importer.import_from_json(args.input)
    
    if len(reports) == 0:
        print("\n❌ No valid reports found in JSON")
        sys.exit(1)
